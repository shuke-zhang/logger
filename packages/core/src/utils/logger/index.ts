...
function info(...msg: unknown[]) {
  ...
  return console.log(`%c[INFO]%c ℹ️ ${message(msg)}`, bg('#00a6ed'), '', ...texts('#00a6ed', msg))
}

function warn(...msg: unknown[]) {
    
    return console.log(`%c[WARN]%c ⚠️ ${message(msg)}`, bg('#ffb02e'), '', ...texts('#ffb02e', msg))
}

function error(...msg: unknown[]) {
 
  return  console.log(`%c[ERROR]%c ❌ ${message(msg)}`, bg('#f92f60'), '', ...texts('#f92f60', msg))
}

function success(...msg: unknown[]) {
    
  return console.log(`%c[SUCCESS]%c ✅ ${message(msg)}`, bg('#0dbc79'), '', ...texts('#0dbc79', msg))
}
